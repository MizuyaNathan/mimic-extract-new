...
def add_outcome_indicators(out_gb):
    ...
    return pd.DataFrame({'subject_id': subject_id, 'hadm_id':hadm_id, 'icustay_id': icustay_id,
...
def continuous_outcome_processing(out_data, data, icustay_timediff):
    ...
    # Convert mapped datetime columns back to datetime objects
    out_data['intime'] = pd.to_datetime(out_data['intime'])
    out_data['outtime'] = pd.to_datetime(out_data['outtime'])
    # Also convert starttime and endtime to datetime
    out_data['starttime'] = pd.to_datetime(out_data['starttime'])
    out_data['endtime'] = pd.to_datetime(out_data['endtime'])
...
def save_numerics(
    ...
    # Ensure value column is numeric, drop any rows where conversion failed
    X = X[pd.to_numeric(X['value'], errors='coerce').notna()]
    X['value'] = pd.to_numeric(X['value'], errors='coerce')

    group_item_cols = ['LEVEL2'] if group_by_level2 else ITEM_COLS
    # Reset index to make ID_COLS regular columns, then select necessary columns for groupby
    X = X.reset_index()
    X = X.groupby(ID_COLS + group_item_cols + ['hours_in'])['value'].agg(['mean', 'std', 'count'])
    # No need to drop level since we're only aggregating 'value' column
...
def save_outcome(
    ...
    vent_data = vent_data.reset_index(drop=True)
    ...
    out_data = out_data.reset_index(level=0, drop=True)
    ...
    for t, c in zip(table_names, column_names):
        ...
        # Use drop=True to avoid adding index columns that might already exist as regular columns
...
if __name__ == '__main__':
    ...
    if (args['extract_numerics'] == 0 | (args['extract_numerics'] == 1) ) & isfile(os.path.join(outPath, dynamic_hd5_filename)):
        ...
        query = \
        """
        select c.subject_id, i.hadm_id, c.icustay_id, c.charttime, c.itemid, c.valuenum as value, valueuom
        FROM icustay_detail i
        ...
        select distinct i.subject_id, i.hadm_id, i.icustay_id, l.charttime, l.itemid, l.valuenum as value, valueuom
        FROM icustay_detail i
